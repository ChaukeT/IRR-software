...
class MainWindow(QWidget):
        def __init__(self, engine, backend, spin_T, rom_cap, r_lo, r_hi, tol, parent=None):
                ...
                self.panels = None
                self.arcs = None
                self.blockmodel_path: Path | None = None
                self.blockmodel_df: pd.DataFrame | None = None
                self._iteration_history: list[tuple[int, float, bool]] = []
        ...
        def on_load(self):
                ...
                try:
                        ...
                        self.blockmodel_df = pd.read_csv(path)
                        self.btn_run.setEnabled(True)
                        self.results_panel.clear()
                        self.results_panel.hide()
                        self.log.append(
                                f"Loaded {len(panels)} panels; built {len(arcs)} arcs from {self.blockmodel_path}"
                        )
        ...
        def on_run(self):
                ...
                def progress_cb(it: int, r: float, feasible: bool, msg: str) -> None:
                        self._iteration_history.append((it, r, feasible))
                        self.log.progress_cb(it, r, feasible, msg)

                info: dict[str, Any] = self.engine.run(
                        panels=self.panels,
                        arcs=self.arcs,
                        econ=econ,
                ...
                if info.get("ok"):
                        ...
                        self.results_panel.update_results(
                                r_star=r_star,
                                schedule=sched,
                                block_df=self.blockmodel_df,
                                iteration_history=self._iteration_history,
                                extra_info=info,
                        )
                else:
                        error_msg = info.get("error") or "Run failed"
                        self.lbl_status.setText("Run failed")
                        self.results_panel.show_error(str(error_msg))
